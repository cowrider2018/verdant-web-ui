...
| `GrainOverlay` | 全幅顆粒／雜訊質感覆蓋層（裝飾件），SVG feTurbulence |
| `FloatingLeaves` | 背景漂浮葉片粒子層（裝飾件），緩慢飄移旋轉 |
| `Marquee` | 無限水平跑馬燈（裝飾件），技能／關鍵字帶 |
| `TiltCard` | 指標驅動 3D 傾斜卡片包裝（裝飾件），可選光澤反光 |
| `AnimatedCounter` | 進入視窗時數字滾動計數，用於成就數據 |
| `Avatar` | 帶葉環裝飾框的個人頭像，無 src 時以佔位符替代 |

> 表單元件 `Field`、`TextInput`、`DateField`、`Select`、`Textarea` 亦由本套件提供。
> 上述裝飾／展示件皆尊重 `prefers-reduced-motion`。
